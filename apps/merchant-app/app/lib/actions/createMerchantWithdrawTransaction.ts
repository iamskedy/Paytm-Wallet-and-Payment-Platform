...
export async function createMerchantWithdrawTransaction(amount: number, provider: string) {
  ...
  const merchant = await db.merchant.findUniqueOrThrow({ where: { email: session.user.email } });
  const token = uuidv4();

  await db.$transaction(async (tx) => {
    const balance = await tx.merchantBalance.findUnique({ where: { merchantId: merchant.id } });
    ...
    await tx.merchantPayoutTransaction.create({
      data: {
  ...
  const payload = {
    token,
    user_identifier: merchant.email,
    amount: parsed.data.amount
  };
  ...
  try {
    const res = await fetch(`${webhookUrl}/hdfcMerchantWithdrawWebhook`, {
      method: "POST",
