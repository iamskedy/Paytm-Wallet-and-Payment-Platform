...
export default async function RootLayout({
  ...
  // Hide sidebar on login page and onboarding pages
  const isAuthPage = !session?.user;
  const isOnboarding = pathname.startsWith("/onboarding");

  // Check if merchant has completed onboarding (has phone number)
  let hasPhone = false;
  if (session?.user?.email) {
    ...
    hasPhone = !!merchant?.number;
  }

  const showSidebar = session?.user && !isOnboarding && hasPhone;
              ...
              <aside className="m-sidebar">
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    fontSize: 18, boxShadow: "0 0 16px rgba(16,185,129,.3)", flexShrink: 0
                  }}>🏢</div>
                  <div>
                    <div style={{ fontFamily: "var(--font-head)", fontSize: 16, fontWeight: 700, letterSpacing: "-.3px" }}>PayFlow</div>
                    <div style={{ fontSize: 9, color: "var(--text3)", fontFamily: "var(--font-mono)" }}>MERCHANT PORTAL</div>
                  </div>
                </div>

                {/* Nav */}
                <nav style={{ flex: 1, padding: "12px 0" }}>
                  <div style={{ padding: "14px 14px 5px", fontSize: 10, fontWeight: 600, color: "var(--text3)", textTransform: "uppercase", letterSpacing: ".08em" }}>Overview</div>
                  <Link href="/dashboard" className="m-nav-item">
                  ...
                  <div style={{ padding: "14px 14px 5px", fontSize: 10, fontWeight: 600, color: "var(--text3)", textTransform: "uppercase", letterSpacing: ".08em" }}>Tools</div>
                ...
                <div style={{ borderTop: "1px solid var(--border)", padding: "12px 8px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{session.user.name ?? "Merchant"}</div>
                      <div style={{ fontSize: 10, color: "var(--text3)", fontFamily: "var(--font-mono)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{session.user.email}</div>
                    </div>
                  </div>
                  <SignOutButton />
                </div>
